...
Farmacias que usan Selenium (JavaScript):  Farmacity, FarmaOnline  → reciben driver en __init__
Farmacias que usan requests (HTML estático): FarmaPlus, Selma, CentralOeste
"""
...
class FarmacityScraper(FarmaciaScraper):
    ...
    Selectores VTEX:
      ...
      - Precio lista:  .vtex-product-price-1-x-listPrice (aparece cuando hay descuento)
    """
...
class FarmaOnlineScraper(FarmaciaScraper):
    """
    FarmaOnline también corre sobre VTEX, por eso los precios vienen
    fragmentados en spans y necesita Selenium para renderizarlos.
...
class CentralOesteScraper(FarmaciaScraper):
    ...
    La página tiene dos span.price seguidos por producto:
      - El primero es el precio final (oferta)
      - El segundo es el precio lista
    Ejemplo: <span class="price">$&nbsp;20.930,00</span>
    ...
    def _scrape(self, url: str) -> tuple[float | None, float | None]:
        ...
        # select() devuelve una lista con TODOS los span.price de la página
