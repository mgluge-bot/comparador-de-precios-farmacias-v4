...
class SelmaScraper(FarmaciaScraper):
    ...
    def _scrape(self, url: str) -> tuple[float | None, float | None]:
        ...
        # Esperamos que cargue el contenedor principal
        self.wait.until(
            EC.presence_of_element_located((By.CSS_SELECTOR, ".v-main"))
        )
        ...
        try:
            main = self.driver.find_element(By.CSS_SELECTOR, ".v-main")

            # Precio oferta: buscamos elementos con "price" en clase dentro del main
            # filtramos el que tenga formato de precio ($XX.XXX)
            price_elems = main.find_elements(By.CSS_SELECTOR, "[class*='price']")
            ...
            # Precio lista: elemento con prev-price en la clase
            prev_elems = main.find_elements(By.CSS_SELECTOR, "[class*='prev-price']")
            if prev_elems:
                lista = limpiar_precio(prev_elems[0].text.strip())
        except Exception:
            pass
