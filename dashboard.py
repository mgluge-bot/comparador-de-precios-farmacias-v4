...
def cargar_datos() -> pd.DataFrame:
    ...
    if len(valores) < 2:
        st.error(f"La hoja tiene menos de 2 filas. Filas encontradas: {len(valores)}")
        return pd.DataFrame()
    headers = valores[0]
    # Debug: mostrar primeras filas para diagnosticar
    st.write("DEBUG headers:", headers[:5])
    st.write("DEBUG primera fila de datos:", valores[1][:5] if len(valores) > 1 else "vacia")
